...
public:
  ...
  void ComputeComplaints(std::vector<CabinetIndex> const &coeff_received, std::vector<CabinetIndex> &);
  bool VerifyComplaintAnswer(CabinetIndex const &from, ComplaintAnswer const &answer);
  void SetQual(std::vector<CabinetIndex> qual);
  ...
  SharesExposedMap ComputeQualComplaints(std::vector<CabinetIndex> const &coeff_received);
  ...
  std::vector<CabinetIndex> const &qual() const;
...
private:
  ...
  std::vector<CabinetIndex> qual_;               ///< Set of qualified members
  ...
  std::unordered_map<CabinetIndex, std::pair<std::set<CabinetIndex>, std::vector<std::unique_ptr<PrivateKey>>>>
