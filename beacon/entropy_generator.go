...
func (entropyGenerator *EntropyGenerator) SetNextAeonDetails(aeon *aeonDetails) {
	...
	incompatibleWithCurrentAeon := numNextAeons == 0 && entropyGenerator.aeon != nil && entropyGenerator.aeon.End > aeon.End
	incompatibleWithNextAeons := (numNextAeons > 0 && entropyGenerator.nextAeons[numNextAeons-1].End > aeon.End)
	if incompatibleWithCurrentAeon || incompatibleWithNextAeons {
