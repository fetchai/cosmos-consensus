...
func (sc *RetrySignerClient) SignDKGMessage(chainID string, dkgMessage *types.DKGMessage) error {
	var err error
	for i := 0; i < sc.retries || sc.retries == 0; i++ {
		err = sc.next.SignDKGMessage(chainID, dkgMessage)
		if err == nil {
			return nil
		}
		time.Sleep(sc.timeout)
	}
	return fmt.Errorf("exhausted all attempts to sign dkg message: %w", err)
}

func (sc *RetrySignerClient) SignEntropy(chainID string, entropy *types.EntropyShare) error {
	var err error
	for i := 0; i < sc.retries || sc.retries == 0; i++ {
		err = sc.next.SignEntropy(chainID, entropy)
		if err == nil {
			return nil
		}
		time.Sleep(sc.timeout)
