...
# usage: ./network_control.sh loss 50% 192.168.1.1 192.168.1.3...
...
if [ "$1" == "reset" ]; then
    ...
    #!/bin/bash
    for i in "${@:3}"; do
        echo "packet loss $i by $2"

        IP_TRANSLATED=`./dns_lookup.sh $i`
        echo "Applying..."

        echo "tc qdisc add dev eth0 parent 1:3 handle 30: netem loss $2"
        tc qdisc add dev eth0 parent 1:3 handle 30: netem loss $2 || true

        echo "tc filter add dev eth0 protocol ip parent 1:0 prio 3 u32 match ip dst $IP_TRANSLATED flowid 1:3"
        tc filter add dev eth0 protocol ip parent 1:0 prio 3 u32 match ip dst $IP_TRANSLATED flowid 1:3
    done
else
    echo "Expected the first argument to the script to be reset or delay or loss. Quitting."
    exit 1
fi
